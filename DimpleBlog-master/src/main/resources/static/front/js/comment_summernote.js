...
function initCommentEditor(space) {
    var editor = ' <div id="CommentEditor">\n' +
        ...
        '                    <div class="feed-sub" style="width: 100%;height:50px;">\n' +
...
function initSimpleMDE(id) {
	summernode = $('#markdownEditor').summernote({
	        placeholder: '说点什么吧',
	        toolbar: [
			  ['style', ['style']],
			  ['font', ['bold', 'italic']],
...
function checkForm() {
    ...
    var htmlText = summernode.summernote('code');
...
function leaveComment() {
    ...
    var data = {
        ...
        content: summernode.summernote('code').trim(),
        notice: $("#notice").val()
    }
    //留言发送
    $.ajax({
